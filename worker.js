...
// MY CODE starts here
const getAtendees = async (meetingId) => {
  const attendees = [];
  let hasMore = true;
  const offsetObject = {};
  ...
  while (hasMore) {
    ...
    while (tryCount <= 4) {
      try {
        searchResult = await hubspotClient.crm.associations.batchApi.read(
          "meeting",
          "contact",
          { inputs: [{ id: meetingId }] }
        );
    ...
    if (!searchResult)
      throw new Error("Failed to fetch attendees for the 4th time. Aborting.");
...
const processMeetings = async (domain, hubId, q) => {
  ...
  while (hasMore) {
    ...
    // This is to get attendees of a meeting
    for (const meeting of data) {
      console.log("syncing meetings attendees for meeting", meeting.id);
      const searchResult = await getAtendees(meeting.id);
      const attendees = searchResult.results || [];
      meeting.properties.attendees = attendees.map(
    ...
    data.forEach(async (meeting) => {
      ...
      const actionTemplate = {
        ...
        meetingProperties: {
          ...
          meeting_attendees: meeting.properties.attendees,
        },
      };
